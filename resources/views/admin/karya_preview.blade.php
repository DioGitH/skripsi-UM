...
<div class="d-flex flex-column">
    <div class=" p-3 d-flex justify-content-between w-100">
        <div class="fw-bold mx-2" style="font-size: 32px">Karya Masuk {{ $jenisKarya->nama }}</div>
...
<form action="{{ route('admin.karya.preview', $jenisKarya->id) }}" method="GET" class="row g-3 align-items-end">
            <div class="col-md-3">
                <label for="kategori" class="form-label">Kategori Karya</label>
                <select name="kategori" id="kategori" class="form-select">
                    <option value="">Semua</option>
                    <option value="siswa" {{ request('kategori') == 'siswa' ? 'selected' : '' }}>Karya Siswa</option>
                    <option value="guru" {{ request('kategori') == 'guru' ? 'selected' : '' }}>Karya Guru</option>
                </select>
            </div>

            <div class="col-md-4">
                <label for="search" class="form-label">Cari Judul / Kreator</label>
                <input type="text" name="search" id="search" class="form-control" placeholder="Cari karya..." value="{{ request('search') }}">
            </div>
